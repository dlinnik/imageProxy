...
- **`frame` (опциональный)**: URL изображения‑рамки.  
  Если задан `frame`, параметры `width`, `height` и `aspect` **игнорируются**.  
  Сервис:
  - загружает рамку и определяет её размер;
  - пропорционально масштабирует основное изображение так, чтобы:
    - сначала ширина стала равна `ширина_рамки - 2 * padding`,
    - если после этого высота больше высоты внутренней области (`высота_рамки - 2 * padding`), масштабирует по высоте до этой высоты;
  - центрирует получившееся изображение внутри рамки с учётом `padding`;
  - накладывает рамку **поверх** итогового изображения.
- **`padding` (опциональный)**: отступ внутрь от внешней границы рамки, в пикселях.  
  - По умолчанию: `0`.  
  - Должен быть неотрицательным целым.
